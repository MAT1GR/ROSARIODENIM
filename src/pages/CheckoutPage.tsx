...
const CheckoutPage: React.FC = () => {
    ...
    const [paymentAmount, setPaymentAmount] = useState<number | null>(null); // Estado para el monto del servidor
    ...
    useEffect(() => {
        ...
        const createPreference = async () => {
            try {
                ...
                if (data.preferenceId && data.totalAmount) {
                    // --- CORRECCIÓN CLAVE: Guardamos ambos valores del servidor ---
    ...
    useEffect(() => {
        if (!preferenceId || paymentAmount === null) return;
        ...
        script.onload = async () => {
            ...
            try {
                ...
                paymentBrickController.current = await bricksBuilder.create("payment", "paymentBrick_container", {
                    initialization: {
                        // --- CORRECCIÓN CLAVE: Usamos el monto exacto del servidor ---
    ...
    }, [preferenceId, paymentAmount, navigate]);
