...
const CheckoutPage: React.FC = () => {
  ...
  // --- INICIO DE LA MEJORA ---
  // Este efecto ahora es mucho más simple.
  useEffect(() => {
    // Solo se ejecuta si tenemos un preferenceId y el script ya está cargado.
      ...
      <div className="container mx-auto max-w-lg px-4">
        <div className="bg-white p-8 rounded-lg shadow-md">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-8">Estás en un entorno seguro. Revisa tu pedido y continúa a Mercado Pago.</p>
          </div>
          ...
          <div className="mb-8 border rounded-lg p-4 space-y-3 bg-gray-50">
            ...
            {cartItems?.map((item: CartItem) => (
              <div key={`${item.product.id}-${item.size}`} className="flex justify-between items-center text-sm">
                <span className="text-gray-800">{item.product.name} <span className="text-gray-500">x{item.quantity}</span></span>
                <span className="font-medium">${(item.product.price * item.quantity).toLocaleString('es-AR')}</span>
              </div>
            ))}
            <div className="flex justify-between items-center text-sm pt-2 border-t">
              <span className="text-gray-800">Envío: {selectedShipping?.name}</span>
              <span className="font-medium">${selectedShipping?.cost.toLocaleString('es-AR')}</span>
            </div>
          ...
          {isLoading && <div className="text-center p-8">Generando link de pago seguro...</div>}
          ...
          <div className={`cho-container flex justify-center transition-opacity duration-300 ${isLoading || error ? 'opacity-0 h-0' : 'opacity-100 h-auto'}`}></div>
        </div>
      </div>
    </div>
  );
