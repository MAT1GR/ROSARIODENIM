...
const Header: React.FC<HeaderProps> = ({ onCartClick }) => {
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-20">
            ...
            <div className="md:hidden">
              <button
                className="p-2 text-brand-primary-text"
            ...
            <nav className="hidden md:flex items-center space-x-8">
              {navLinks.slice(0, 2).map((link) => (
                <Link
                  key={link.href}
                  to={link.href}
                  className={`font-poppins text-sm font-medium text-brand-primary-text hover:text-black relative after:content-[''] after:absolute after:w-full after:h-[1px] after:bottom-[-4px] after:left-0 after:bg-black after:transition-transform after:duration-300 ${
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
              <Link
                to="/"
                className="font-poppins text-2xl tracking-[0.2em] text-gray-800 uppercase"
              >
                <span className="font-semibold text-black">denim</span>
                <span className="font-light text-gray-500">rosario</span>
              </Link>
            </div>
            ...
            <div className="flex items-center justify-end">
              ...
              <div className="flex items-center space-x-2 ml-4">
                ...
                <button
                  onClick={onCartClick}
                  className="relative p-2 text-brand-primary-text hover:text-black"
                >
                  <ShoppingBag size={20} />
                  {totalItems > 0 && (
      ...
      >
        <div
          className={`absolute top-0 left-0 h-full w-3/4 max-w-xs bg-white shadow-xl p-6 transition-transform duration-300 ${
